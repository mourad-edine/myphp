...
class ClientController extends Controller
{
    ...
    public function validation(Request $request){
        ...
        $total = $mois * $prix;
        dd($mois,$prix,$total);*/
        $prof = $request->id_prof;
        $client = Client::findOrFail($request->id);
        $payement = [
           $prof => ['payement'=> 1]
        ];
        $client->profs()->sync($payement);
        //dd($request->id,$request->id_prof,$request->nombre);
        if($client){
            return back()->with('payer','payement éffectué avec success');
        }else{
            return back()->with('pas','erreur');
